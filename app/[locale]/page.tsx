...
const listingLabel = (type: ListingType, t: (key: string) => string) =>
...
export default async function HomePage({ params }: { params: Params }) {
  const { locale } = params;
  const baseT = await getTranslations({ locale });
  const t = (key: string, values?: Record<string, any>) => baseT(`home.${key}`, values);
  ...
  return (
    <div className="space-y-12">
      <Script id="home-schema" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} />
